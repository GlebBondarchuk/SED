...
<div class="row">
    <div class="col-lg-12">
        ...
        <a class="btn btn-danger btn-primary" href="<c:url value="${applicationPath}/user/${username}/edit"/>" role="button"><spring:message code="people.button.cancel"/></a>
    </div>
</div>
<script>
    function addTab() {
        $.ajax({
            type: "POST",
            url: "<c:url value="${applicationPath}/user/${username}/addTab"/>",
            data: {contentName: $("#contentName").val(), content: $("#htmlContent").val()}
        }).done(function () {
            window.location.href = "<c:url value="${applicationPath}/user/${username}/edit"/>";
        })
    }
    ...
    function editTab() {
        $.ajax({
            type: "POST",
            url: "<c:url value="${applicationPath}/user/${username}/edit/tab/${content.id}"/>",
            data: {contentName: $("#contentName").val(), content: $("#htmlContent").val()}
        }).done(function () {
            window.location.href = "<c:url value="${applicationPath}/user/${username}/edit"/>";
        })
